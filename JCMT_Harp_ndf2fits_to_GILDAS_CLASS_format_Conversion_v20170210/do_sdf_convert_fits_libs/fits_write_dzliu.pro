pro fits_write_dzliu,file_or_fcb,data,header_in,extname=extname,extver=extver, $
...
;
	...
	if npad eq 2880 or keyword_set(no_pad) then npad = 0
	if npad gt 0 then writeu,fcb.unit,bytarr(npad) ; <dzliu> modified and allowed user to decide not pad 2880*N bytes
;
...
;
			...
			fits_write_dzliu,fcb,0,hmain,/no_abort,message=message
			if message NE '' then goto,error_exit
	    end
	end
;
...
;
	    ...
	    if unsigned then writeu,fcb.unit,newdata else writeu,fcb.unit,data
            if npad gt 0 and ~keyword_set(no_pad) then begin
                if Axtension EQ 'TABLE   ' then padnum = 32b else padnum = 0b
	        writeu,fcb.unit,replicate(padnum,npad)
            endif ; <dzliu> modified and allowed user to decide not pad 2880*N bytes
